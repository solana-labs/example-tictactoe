...
SOL_FN_PREFIX bool game_join(
  ...
  if (self->game_state != GameState_Waiting) {
    sol_print(2, self->game_state, 0, 1, 1);
    ...
    case GameState_OMove:
      if (game_same_player(player, &self->player_x)) {
        if (timestamp <= self->keep_alive[0]) {
          sol_print(2, timestamp, self->keep_alive[0], 1, 3);
          // Result_InvalidTimestamp;
          return false;
        }
        sol_print(2, timestamp, 0, 2, 3);
      ...
      if (game_same_player(player, &self->player_o)) {
        if (timestamp <= self->keep_alive[1]) {
          sol_print(2, timestamp, self->keep_alive[1], 3, 3);
          // Result_InvalidTimestamp;
          return false;
        }
        sol_print(2, timestamp, 0, 4, 3);
        self->keep_alive[1] = timestamp;
        return true;
      }
      sol_print(2, 0, 0, 5, 3);
...
extern bool entrypoint(const uint8_t *input) {
  ...
  if (instruction_data_len < sizeof(uint32_t) + sizeof(CommandData)) {
    sol_print(0, 0, instruction_data_len, sizeof(uint32_t) + sizeof(CommandData), 3);
    return false;
  }
  ...
  if (ka[1].userdata_len < sizeof(uint32_t) + sizeof(StateData)) {
    sol_print(0, cmd, ka[1].userdata_len, sizeof(uint32_t) + sizeof(StateData), 4);
    return false;
  }
